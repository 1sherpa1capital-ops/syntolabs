    ...
    const cards = [
        {
            ...
            visual: (
                <div className="relative flex items-center justify-center">
                    {/* Central Hub */}
                    <div className="relative z-10 bg-surface-elevated rounded-2xl shadow-lg px-5 py-3 flex items-center gap-2 border border-border">
        ...
        },
        {
            ...
            visual: (
                <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
                    ...
                    {/* Floating Callers */}
                    <div className="absolute top-4 right-10 bg-surface-elevated shadow-md border border-border p-1.5 rounded-lg flex gap-1.5 items-center animate-bounce-gentle">
                        <PhoneIncoming size={10} className="text-primary" />
                        <div className="w-8 h-1 bg-border rounded-full" />
                    </div>
                    <div className="absolute bottom-6 left-10 bg-surface-elevated shadow-md border border-border p-1.5 rounded-lg flex gap-1.5 items-center animate-bounce-gentle" style={{ animationDelay: '0.5s' }}>
                        <PhoneIncoming size={10} className="text-primary" />
                        <div className="w-6 h-1 bg-border rounded-full" />
                    </div>
                    
                    {/* Success Badge */}
                    <div className="absolute -top-2 left-1/2 -translate-x-1/2 bg-accent text-white px-2 py-0.5 rounded text-[9px] font-bold shadow-sm">
                        No Hold Time
                    </div>
                </div>
            )
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                    {cards.map((card, i) => (
                        <ThreeCardWrapper key={i} className="flex flex-col">
                            <div className="flex flex-col h-full bg-surface border border-border rounded-xl overflow-hidden transition-all duration-300 hover:border-accent/30 group">
                                {/* Visual Area - Compact Height */}
                                <div className={`relative h-32 md:h-40 flex items-center justify-center p-4 overflow-hidden rounded-t-xl ${card.gradient === 'green' || card.gradient === 'blue' ? 'card-gradient-blue' : 'card-gradient-teal'} group-hover:scale-[1.02] transition-transform duration-500`}>
                                    {card.visual}
                                </div>

                                {/* Content Area - Tighter Padding */}
                                <div className="p-5 flex flex-col flex-1">
                                    <h3 className="text-base md:text-lg font-bold text-dark mb-2 leading-tight group-hover:text-primary transition-colors">{card.title}</h3>
